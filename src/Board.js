...
function Board({ nrows = 5, ncols = 5, chanceLightStartsOn = 0.25 }) {
  ...
  function createBoard() {
    ...
    for (let y = 0; y < nrows; y++) {
      ...
      for (let x = 0; x < ncols; x++) {
        row.push(Math.random() < chanceLightStartsOn); // Randomly decide if a cell is lit
      }
  ...
  /** check if the player has won (all cells off) */
  function hasWon() {
  ...
  function flipCellsAround(coord) {
    setBoard(oldBoard => {
      ...
      const boardCopy = oldBoard.map(row => [...row]); // Create a deep copy of the board
